...
name: debug
description: >-
  General debugging for control panel, Discord, worker/ARQ, Postgres/Redis,
...
```text
/debug → /fix           control panel, infra, imports, CLI startup
...
| ------ | ---------------- | ---------------- |
| **A. Control panel** | Discord slash fails, job stuck, no preview, webhook | control-panel logs, Postgres jobs, Redis, worker |
| **B. Infra / env** | Process won't start, DB password, Redis down | `.env`, `.control-panel.yml`, `docker ps`, traceback |
...
When? (fresh vs stale)
Control panel up? (panel + worker)
Env/config paths: .env, .control-panel.yml
```
...
4. .control-panel.yml — discord.enabled, guild_ids, sync_joined_guilds
...
# Quick reference — local control panel

```text
scripts/start-control-panel.ps1  → Redis, worker, API :8787
.env + .control-panel.yml
