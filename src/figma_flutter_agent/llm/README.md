...
IR generate (`use_screen_ir: true`) also injects report-only semantic context via `semantic_context.assemble_semantic_context`: `rawContext`, `treeOutline`, `textInventory`, `componentInventory`, `geometryInventory`, and `relationshipHints` in the user payload. The model may return `screenIr.semanticSummary` / `screenIr.semanticVerdicts` with contract-oriented fields (`contractKind`, `contractTraits`, node id partitions, `proposedLayoutLaws`, `proposedEffects`); these are debug/report data only and are not applied by emit or repair.
