...
_AGENT_WIDGET_MARKER = "Generated by figma-flutter-agent"


def prune_disk_unplanned_agent_widgets(
    project_dir: Path,
    planned: dict[str, str],
) -> list[str]:
    """Delete agent-generated widget files on disk that are absent from the planned catalog."""
    widgets_dir = project_dir / "lib" / "widgets"
    if not widgets_dir.is_dir():
        return []

    planned_paths = {path.replace("\\", "/") for path in planned}
    removed: list[str] = []
    for dart_file in sorted(widgets_dir.glob("*.dart")):
        rel = f"lib/widgets/{dart_file.name}"
        if rel in planned_paths:
            continue
        try:
            head = dart_file.read_text(encoding="utf-8")[:400]
        except OSError as exc:
            logger.warning("Could not read widget file {}: {}", rel, exc)
            continue
        if _AGENT_WIDGET_MARKER not in head:
            continue
        try:
            dart_file.unlink()
            removed.append(rel)
            logger.info("Removed unplanned agent widget on disk: {}", rel)
        except OSError as exc:
            logger.warning("Could not remove unplanned agent widget {}: {}", rel, exc)
    return removed
