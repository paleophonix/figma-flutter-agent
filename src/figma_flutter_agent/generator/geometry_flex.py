...
from figma_flutter_agent.generator.layout_flex_policy import FlexWrapKind, resolve_flex_wrap
...
def compute_input_metrics(node: CleanDesignTreeNode) -> TextMetricsFrame | None:
    ...
    leading = _leading_above_flutter(float(font_size), ratio)
...
def compute_flex_deltas(
    ...
    if child.type == NodeType.INPUT:
        ...
        if parent_type == NodeType.COLUMN and child.sizing.width_mode == SizingMode.FILL or parent_type == NodeType.ROW and child.sizing.width_mode == SizingMode.FILL:
            wraps.append(WrapKind.EXPANDED)
