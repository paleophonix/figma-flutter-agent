...
def _chunk_dart_file_stem(feature_name: str, class_name: str) -> str:
    """Return the file stem for a chunk Dart file.

    Args:
        feature_name: Layout feature name (e.g. ``"background"``).
        class_name: Chunk class name (e.g. ``"FigmaChunkA3F9C21B"``).

    Returns:
        File stem without extension, e.g. ``"background_chunk_a3f9c21b"``.
    """
...
def render_widget_file(
    *,
    ...
    dart_ui_import = _dart_ui_import_line(body)
    artboard_preview_fields = (
        ARTBOARD_PREVIEW_CLASS_FIELDS
        if ARTBOARD_PREVIEW_LAYOUT_MARKER in body
        else ""
    )
...
"""
    ...
    if chunk_bodies:
        ...
        for cn, _ in chunk_bodies:
            stem = _chunk_dart_file_stem(feature_name, cn)
            chunk_import_lines += (
        ...
        for cn, body in chunk_bodies:
            dart_ui = _dart_ui_import_line(body)
            svg = (
                "import 'package:flutter_svg/flutter_svg.dart';\n\n"
            ...
            chunk_stem = _chunk_dart_file_stem(feature_name, cn)
