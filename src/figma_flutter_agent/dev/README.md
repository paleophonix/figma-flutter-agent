...
- `dev/preview_size.py` — wizard **launch** reads the layout dump size first, falling back to `responsive.preview_width` / `preview_height` from `.ai-figma-flutter.yml` only when no dump is available. `responsive.mode` selects preview: `static` (fixed Figma artboard dart-defines), `responsive` (wide adaptive preview), or `both` (two Chrome windows; terminal and render-error logs attach to the responsive instance only). It avoids Chromium `--window-size` because comma-separated values can be misread as URLs. Chrome `flutter run` adds `--no-web-resources-cdn` so CanvasKit loads locally when gstatic is blocked.
