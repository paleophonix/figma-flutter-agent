...
- `dev/preview_size.py` — wizard **launch** reads the layout dump size first, falling back to `responsive.preview_width` / `preview_height` from `.ai-figma-flutter.yml` only when no dump is available. `responsive.enabled` selects the mode: `true` launches wide responsive preview without artboard dart-defines, `false` launches fixed Figma preview with artboard dart-defines. It avoids Chromium `--window-size` because comma-separated values can be misread as URLs. Chrome `flutter run` adds `--no-web-resources-cdn` so CanvasKit loads locally when gstatic is blocked.
