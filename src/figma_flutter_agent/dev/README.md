...
- `dev/preview_size.py` — wizard **launch** reads `responsive.preview_width` / `preview_height` from `.ai-figma-flutter.yml` (else layout dump), passes `--window-size`, and passes artboard dart-defines for fixed artboard preview (`adaptive_render: false`, or explicit preview size). Capture stays artboard 1:1 always.
