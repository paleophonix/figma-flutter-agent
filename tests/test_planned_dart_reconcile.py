...
def test_prune_disk_unplanned_agent_widgets_removes_stale_generated_file(tmp_path) -> None:
    widgets = tmp_path / "lib" / "widgets"
    widgets.mkdir(parents=True)
...
"""
    (widgets / "success_widget.dart").write_text(stale, encoding="utf-8")
    planned = {
        "lib/widgets/right_button_widget.dart": """
import 'package:flutter/material.dart';

class RightButtonWidget extends StatelessWidget {
  const RightButtonWidget({super.key});
  @override
  Widget build(BuildContext context) => const SizedBox();
}
"""
    }
    removed = prune_disk_unplanned_agent_widgets(tmp_path, planned)
    assert "lib/widgets/success_widget.dart" in removed
    assert not (widgets / "success_widget.dart").exists()
