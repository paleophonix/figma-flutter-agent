"""Tests for demo ``.figma_debug/reference`` emitter golden bundle."""
...
def _demo_project_dir() -> Path | None:
    for candidate in _DEMO_ROOT_CANDIDATES:
        ref = candidate / ".figma_debug/reference" / f"{_FEATURE}_screen.dart"
        if ref.is_file():
            return candidate
    return None
...
def test_background_reference_bundle_has_expected_sections() -> None:
    ...
    bundle = (
        project_dir / ".figma_debug/reference" / f"{_FEATURE}_screen.dart"
...
def test_background_reference_metadata_points_at_bundle() -> None:
    project_dir = _demo_project_dir()
    assert project_dir is not None
    meta_path = project_dir / ".figma_debug/reference" / f"{_FEATURE}_reference.json"
    payload = json.loads(meta_path.read_text(encoding="utf-8"))
    assert payload["bundle"].endswith(f"reference/{_FEATURE}_screen.dart")
