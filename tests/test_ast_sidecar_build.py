...
def test_ast_sidecar_preflight_none_when_prebuilt_exists(tmp_path: Path) -> None:
    ...
    settings = Settings()
    with patch("figma_flutter_agent.dev.ast_sidecar_build.agent_repo_root", return_value=tmp_path):
        with patch(
...
def test_ast_sidecar_preflight_when_binary_missing(tmp_path: Path) -> None:
    ...
    settings = Settings()
    with patch("figma_flutter_agent.dev.ast_sidecar_build.agent_repo_root", return_value=tmp_path):
        with patch("figma_flutter_agent.dev.ast_sidecar_build._prebuilt_compiler_path", return_value=None):
            with patch(
