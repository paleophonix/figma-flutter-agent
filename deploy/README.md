...
```bash
docker compose -f deploy/docker-compose.vps.yml --profile bundled-db up -d --build
```
...
| `deploy/Caddyfile` | TLS reverse proxy to `control-panel:8787` |
...
| Task | Command |
| --- | --- |
| Logs | `docker compose -f deploy/docker-compose.vps.yml logs -f control-panel arq-worker` |
| Restart | `docker compose -f deploy/docker-compose.vps.yml restart control-panel arq-worker` |
