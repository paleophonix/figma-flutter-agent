...
- Local fixture warm capture: `FIGMA_FLUTTER_PROJECT_DIR` → `project/.debug/capture/sandbox` via `validation/golden_capture/warm_runtime.py` (`FixtureCaptureBatch`); screen-run perf lives under `<project>/.debug/<feature>/secondary/perf.json`; fixture-only perf may still use `logs/perf/`
...
5. Golden: `scripts/update-golden-docker.ps1` or pipeline refine; compare `<project>/.debug/<feature>/primary/figma.png` vs the Flutter render/diff under `<project>/.debug/<feature>/secondary/capture/`.
...
Current `.debug` layout is screen-centric:
...
Naming rule: the feature slug lives in the directory path; files inside `primary/` and `secondary/` use short stable names. Example: `feedback/primary/screen.dart`, not `dart/feedback_screen.dart`.

Primary triage read order:

```text
raw.json → processed.json → pre_emit.json → screen.dart → figma.png → semantics.json
```

Project-level artifacts:

- `logs/last.log`: exact command transcript, subprocess stdout/stderr, analyzer JSON blocks.
- `sync/snapshot.json`: incremental sync hashes and drift.
- `wizard-state.yml`: active screen slug.
- `pubspec_resolve.sha256`: last successful `pub get` stamp.
- `capture/sandbox/`: shared warm golden capture sandbox.

Deprecated v2 paths are not canonical for new work: `raw/<feature>_layout.json`, `processed/<feature>_layout.json`, `ir/<feature>_*.json`, `dart/<feature>_*.dart`, `semantics/<feature>.json`, `provenance/<feature>.json`, `reports/<feature>_*.json`, `reference/figma/<feature>_figma.*`, root-level `renders/`, root-level `perf/`, and `.artifact-layout-v2`.
